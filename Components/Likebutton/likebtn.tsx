...
const Likebtn = ({ likedValue, postId, likedUsers }: LikeBtnProps) => {
  ...
  const handlelike = async () => {
    ...
    try {
        const endpoint = isLiked ? `http://social-media-rest-apis-1.onrender.com/api/posts/like/${postId}` :
        `http://social-media-rest-apis-1.onrender.com/api/posts/unlike/${postId}`
        ...
        setLike(prevLiked => isLiked ? prevLiked + 1 : prevLiked - 1)
        setIsLiked(isLiked)
    }
